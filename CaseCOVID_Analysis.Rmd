...
View the full data preprocessing pipeline [here](https://github.com/ByronSyun/DIVAS_COVID19_CaseStudy)
...
**How to read the next two sections.** *Preparing Data Blocks* and *Running DIVAS Analysis* show the exact commands you would run on your own data, but they are **not executed when this page is rendered**, because a full six-block run takes around 5 hours. What this vignette actually runs is *Loading Pre-computed Results* further below, which reads a saved result object. Use the two sections below as a template for your own analysis, then pick the tutorial back up at *Loading Pre-computed Results*.

**Why 5 hours?** DIVAS does a comprehensive search among candidate components corresponding to all possible combinations of modalities. In our 6-modality case this means evaluating 1 6-way, 6 5-way, 15 4-way, 20 3-way, 15 2-way and 6 1-way combinations (63 in total), which is also why runtime grows quickly with the number of modalities.
...
# pdf("figs/divas_diagnostic_plots.pdf", width = 12, height = 8)
# 
...
For this tutorial, we use pre-computed DIVAS results. **Download the required data from Zenodo** and place them in the appropriate directory.

**Zenodo Dataset:** 
<a href="https://doi.org/10.5281/zenodo.17430294">
  <img src="https://zenodo.org/badge/DOI/10.5281/zenodo.17430294.svg" alt="DOI">
</a>

**Required files:**
1. `divas_results_combined_6block_renamed.rds` (419 MB) - Download from [Zenodo](https://zenodo.org/records/17430294)
   - Place in: `scRNA_celltyist_analysis/DIVAS_run/DIVAS_Results/`
2. `metadata.rds` (4.1 KB) - Already included in GitHub repo ✓

```{r load-results}
# Load pre-computed DIVAS results and clinical metadata
# Download divas_results_combined_6block_renamed.rds from:
# https://zenodo.org/records/17430294
# Place in: scRNA_celltyist_analysis/DIVAS_run/DIVAS_Results/

divasRes <- readRDS("scRNA_celltyist_analysis/DIVAS_run/DIVAS_Results/divas_results_combined_6block_renamed.rds")
metaDat <- readRDS("scRNA_celltyist_analysis/DIVAS_run/DIVAS_Results/metadata.rds")
```


The first key output from DIVAS is the rank decomposition plot. This plot is produced by `DIVASmain()` itself, as one of its diagnostic outputs: it is drawn when `iprint = TRUE` (the default) and is written to file when a directory is supplied via `figdir`. There is no separate plotting call, and it is not produced by the loadings commands used later in this vignette. It appears below as a saved image only because the `DIVASmain()` call above is not executed when this page is rendered.
...
matrixPlot(df_sc, comp_idx = 1:4, colBy = metaDat$`Who Ordinal Scale`, pointSize = 2, fsize = 8, manualCol = colScal)
matrixPlot(df_sc, comp_idx = 5:8, colBy = metaDat$`Who Ordinal Scale`, pointSize = 2, fsize = 8, manualCol = colScal)
```
...
```{r fig.asp=1, fig.width=12, fig.height=12}
matrixPlot(df_sc, var_names = names(impSc_sort)[1:5], colBy = metaDat$`Who Ordinal Scale`, pointSize = 2, manualCol = colScal, fsize = 6)
```
...
matrixPlot(df_sc, var_names = "CD8+metab-5", colBy = metaDat$`Who Ordinal Scale`, pointSize = 2, manualCol = colScal, fsize = 10) +
  xlim(-0.1, 0.1)
```
...
```{r get-top-features-cd4}
# Extract top features from the #1 ranked component (most correlated with severity)
# This is a 5-way component: CD4+CD14+NK+pro+metab-1
comp_name <- "CD4+CD14+NK+pro+metab-1"
...
```{r go-enrichment-cd4, fig.asp=1.8}
...
```{r go-enrichment-cd14, fig.asp=1.8}
...
input_dir <- "scRNA_celltyist_analysis/DIVAS_run/divas_input_T1T2combined"
data_blocks_files <- c("CD4_T_combined", "CD14_Monocyte", "NK", "proteomics", "metabolomics")

data_list_circos <- lapply(data_blocks_files, function(block_name) {
  file_path <- file.path(input_dir, paste0("Combined_", block_name, ".csv"))
  read.csv(file_path, row.names = 1, check.names = FALSE)
})

# Use short names for DIVAS (matching renamed results)
modNames <- c("CD4", "CD14", "NK", "pro", "metab")
names(data_list_circos) <- modNames
...
feature_list_circos$pro <- sapply(feature_list_circos$pro, function(x) {
...
protBlock <- data_list_circos$pro
...
data_list_circos$pro <- protBlock
```

```{r circos-plot, fig.width=8, fig.height=8}
...
**Note**: This analysis used pre-computed DIVAS results.

**To run DIVAS on your own data**, start from the [Toy Dataset vignette](https://byronsyun.github.io/DIVAS/articles/DIVAS_Toy_Dataset_Example.html) in the DIVAS package documentation. It walks through preparing a list of matched data matrices, running `DIVASmain()`, and interpreting the signal extraction stage, including the estimated noise level, initial and bootstrap-refined signal ranks, and perturbation angles.

**To reproduce the preprocessing for this case study**, see the `scRNA_celltyist_analysis/` directory of this repository, which contains the CellTypist cell type annotation and the T1/T2 sample preparation specific to the COVID-19 dataset. Note the directory name is spelled `scRNA_celltyist_analysis` (without the "p" of "celltypist"); searching for `scRNA_celltypist_analysis` will not find it.
