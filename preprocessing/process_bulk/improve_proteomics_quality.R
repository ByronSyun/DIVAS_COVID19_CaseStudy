...
# Improve Proteomics Data Quality
#
# This script performs the following steps:
# 1. Assesses the distribution of missing values.
# 2. Filters out proteins with a high percentage of missing values (>20%).
...
cat("Starting proteomics data quality improvement...\n")

# Define paths
# Assumes the script is run from the 'process_bulk' directory
data_dir <- "."
output_dir <- "improved_proteomics"
...
data_path <- file.path(data_dir, "proteomics_data.csv")
if (!file.exists(data_path)) {
    stop(paste("Input data file not found:", data_path))
}
proteomics_data <- read.csv(data_path, row.names = 1, check.names = FALSE)
cat("Loaded proteomics data. Dimensions:", dim(proteomics_data), "\n")

# --- 1. Missing Value Assessment ---
cat("Assessing missing values...\n")
na_count_per_protein <- rowSums(is.na(proteomics_data))
na_percent_per_protein <- na_count_per_protein / ncol(proteomics_data) * 100

# --- 2. Filter Proteins ---
missing_threshold <- 20
proteins_to_keep <- na_percent_per_protein <= missing_threshold
original_protein_count <- nrow(proteomics_data)
filtered_data <- proteomics_data[proteins_to_keep, ]
retained_count <- nrow(filtered_data)
cat("Filtered out", original_protein_count - retained_count, "proteins with >", missing_threshold, "% missing values.\n")
cat("Retained", retained_count, "proteins.\n")
...
output_file_path <- file.path(output_dir, "improved_proteomics_data.csv")
write.csv(normalized_data, output_file_path)
cat("Saved improved proteomics data to:", output_file_path, "\n")
...
pca_plot <- ggplot(score_data, aes(x = PC1, y = PC2, color = Group)) +
  ...
  theme_bw() +
  labs(title = "PCA of Improved Proteomics Data",
...
# Save PCA plot
pca_plot_path <- file.path(output_dir, "pca_plot_proteomics.png")
ggsave(pca_plot_path, pca_plot, width = 8, height = 6)
cat("Saved PCA plot to:", pca_plot_path, "\n")

cat("Proteomics data quality improvement complete.\n") 